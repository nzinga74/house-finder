...
const Contract = () => {
  ...
  const initialData = async () => {
    if (users != null) {
      const allcontracts = await getcontrats(users?.id);
      if (allcontracts) {
              ...
              {contracts.length > 0 && (
               ...
               `}
                  ...
                  <p>
                    ...
                    {selectedContract?.transaction_price}), a ser pago da
                    seguinte forma:
                  </p>
                  <h4>PRAZO DE ENTREGA</h4>
                  <p>
                    O Vendedor concorda em entregar o Imóvel ao Comprador até
                    [Data de entrega].
                  </p>
                  <h4>OBRIGAÇÕES DAS PARTES</h4>
                  <p>
                    O Vendedor se compromete a transferir a propriedade do
                    Imóvel ao Comprador mediante o pagamento integral.
                  </p>
                  <p>
                    O Comprador concorda em efetuar os pagamentos conforme
                    acordado e em cumprir com todas as obrigações contratuais.
                  </p>
                </>
              )}
            </div>
